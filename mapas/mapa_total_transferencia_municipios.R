...
gerar_mapa <- function(){
  ...
  pal_fun <- colorBin("viridis", 
                      domain = shp_total_transf$total_transferido, 
                      bins = map_breaks, 
                      pretty = TRUE, 
                      reverse = TRUE)
  ...
  labels <- sprintf(
    ...
    shp_total_transf$descricao, formata_moeda(shp_total_transf$total_transferido)
  ...
  leaflet(shp_total_transf) %>%
    ...
    addPolygons(
      stroke = TRUE,
      fillColor = ~pal_fun(total_transferido),
      fillOpacity = 0.8,
      weight = 1,
      opacity = 1,
      color = "white",
